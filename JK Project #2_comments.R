...
#CPK: Yes, for this  we needed to group by date bc cycles refer to each experiment, not across them. But, why is so much of this ^ being repeated? [-1]. Not sure why you're computing mean on single rows.

pseed.sum.max<- pseed.max %>%
  group_by(fish, speed,bl.s) %>%
  summarize(amp.sum.mean=mean(amp.sum),amp.sum.se = stderror(amp.sum)) 

#CPK:^^Good on ya for thinking to combine the mean and se operations, however, this throws and error bc you haven't yest defined the functon stederror. [-1]. I addes "bl.s" to group_by so we could merge with the met data.
...
#CPK: Why not col the error bars by fish, too?
...
#CPK: Commenting this ^^ out bc it's unneeded and prevets tibbles from merging. 
...
#CPK: This^^^was tough bc you didn't preserve the bl.s column in pseed.sum.max. But, could have been this easy if you had.

pseed.sum.max%>%
  left_join(pseed.met.rate%>%
              group_by(fish,bl.s)%>%
              summarize(amp.met.rate=mean(met.rate))) %>% 
  ggplot(aes(x=amp.sum.mean, y=amp.met.rate, colour=fish)) + geom_point()
...
#CPK: Pretty good work. Getting there! Just need to get a better grasp on when to you to pipe to become more concise and address the project propmpts
